        ...
        <div class="panel-body">
          <el-table :data=" tasks" stripe v-loading="loading" element-loading-text="正在停止检测任务">
            ...
            <el-table-column label="检测类型" align="center">
              <template slot-scope="scope">
                <p class="text-center" v-show="scope.row.isFinger == 0">
                源代码</p>
                <p class="text-center" v-show="scope.row.isFinger == 1">
                  指纹</p>
              </template>
            </el-table-column>
            <el-table-column label="检测状态" align="center">
              <template slot-scope="scope">
                ...
                <el-progress v-show="scope.row.status==1" :text-inside="true" :stroke-width="18"
            ...
            <el-table-column label="操作" align="center">
              <template slot-scope="scope">
                <el-tag type="info" v-show="scope.row.status == 2" size="medium">
                  <router-link :to="{path: scope.row.id + '/pair_clone' }">查看</router-link>
                </el-tag>
                <el-button v-show="scope.row.status == -1 || scope.row.status == 1" type="info" plain size="mini"
                           disabled>
                  查看
                </el-button>
...
<script>
  export default {
    ...
    methods: {
      ...
      getPagedTaskList() {
        this.$http.get('/api/tool/pair_task/user', {
          params:
            {
              id: this.user_id,
      ...
      deleteTask: function (row) {
        ...
        this.$confirm(msg, '提示', {
          ...
          this.$http.delete('/api/tool/pair_task/' + row.id).then(() => {
            this.$message({
              type: 'success',
              message: '删除成功!',
